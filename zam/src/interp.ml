  ...
  while !pc < Array.length code do
    ...
    begin
      ...
      | 44 (* CLOSUREREC *) -> 
         ...
         for i = 1 to f-1 do 
           Block.set_field !Domain.acc (2 * i - 1) (Block.make_header Block.infix_tag (2 * i));
           Block.set_field !Domain.acc (2 * i) (Mlvalues.val_long (take_argument code))
         done;
         for i = 0 to v-1 do 
      ...
      | 55 (* GETGLOBALFIELD *) -> let n = take_argument code in
                                   ...
                                   let g = Block.get_global n in
                                   Domain.acc := Block.get_field g p
      ...
      | 79 (* VECTLENGTH *) -> Domain.acc := Mlvalues.val_long (Block.size (Mlvalues.ptr_val (!Domain.acc)))

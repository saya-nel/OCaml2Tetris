...
| Bc.IfGoto l -> Some (Kbc.IfGoto (match List.assoc_opt l labels with 
                                   | None -> assert false
                                   | Some x -> x))
| Bc.Function ((m,f),n) -> if (m,f) = ("Main","main") then main_main_pos := Some (pc,n);
                           None
...
| Bc.Call(("ML_Internal","array_create_uninitialized"),_) -> Some (Kbc.Prim (ML_array_create_uninitialized))
...
| Bc.Call(("ML_Internal","print_char_array"),_) -> Some (Kbc.Prim (ML_print_char_array))
...
| Bc.Call(mf,arity) -> let pc,nblocals = match List.assoc_opt mf functions with

...
let rec rw_exp cl env lenv e = 
  ...
  | Ast.Match(e,ms) ->  Ast.Match (rw_exp cl env lenv e,
                                   List.map (fun m -> match m with
                                                      | Ast.Case(c,e) -> Ast.Case(c,rw_exp cl env lenv e) 
                                                      | Ast.Otherwise(e) -> Ast.Otherwise(rw_exp cl env lenv e)) ms) 
  | _ -> e
...
and rw_fundecs cl lf = 
  List.map (fun d -> match d with | Ast.DF (name,args,e) -> Ast.DF (name,args,rw_exp cl [] [] e)) lf

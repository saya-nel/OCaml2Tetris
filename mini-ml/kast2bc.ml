...
let nb_local bc =
  List.fold_left
    (fun n -> fun u -> match u with
               | Bc.Pop(seg) -> (match seg with Bc.Local(i) -> Pervasives.max (i+1) n | _ -> n)
               | _ -> n) 0 bc
...
let next_closure bc_e adr = 
  ...
  lambda_code := (Bc.Function (f,nb_local bc_e) :: []) @ bc_e @ (Bc.Return :: []) @ (!lambda_code);
  apply_code :=  Bc.Push(Bc.Argument(0)) ::   (* arg0 : [|code;addr;vars...|] *)
                 ...
                 Bc.Push(Bc.Argument(0)) :: (* la raison pour laquel l'environnement est passé en 2ème (et pas en premier) *)
...
let bc_of_variable v = match v with
  | Kast.Global(name) -> 
    (Bc.Call (name,0)) :: [] (* les variables globales sont des fonction globales d'arité 0  *) 
